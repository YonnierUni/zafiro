...
export function LocationSection({ business, dictionary, locale }: LocationSectionProps) {
  return (
    <section className="mx-auto max-w-7xl px-5 py-20 sm:px-6 sm:py-24 lg:px-8 xl:max-w-[90rem] 2xl:px-10">
      <div className="grid gap-8 lg:grid-cols-12 lg:gap-10 xl:gap-12">
        ...
        <div className="grid gap-4 sm:gap-5 lg:col-span-8 lg:grid-cols-8 lg:grid-rows-[minmax(15rem,1fr)_minmax(13rem,1fr)] xl:gap-6">
          <div className="interactive-card rounded-[1.75rem] border border-white/10 bg-[radial-gradient(circle_at_top,_rgba(36,107,255,0.16),_transparent_38%),rgba(255,255,255,0.04)] p-5 shadow-glow sm:p-6 lg:col-span-5 lg:row-span-2 xl:p-7">
            <p className="text-xs uppercase tracking-[0.3em] text-cyanGlow/80">{dictionary.location.where}</p>
            <h3 className="mt-4 font-display text-3xl text-ivory sm:text-[2.35rem] xl:text-[2.9rem]">
              {business.city}, {business.region}
            </h3>

            <div className="mt-6 grid gap-4 text-sm leading-6 text-mist xl:grid-cols-2">
            ...
            <p className="mt-6 border-t border-white/10 pt-5 text-sm leading-6 text-mist xl:max-w-[38ch] xl:text-[1rem] xl:leading-7">
              {dictionary.location.whereDescription}
            </p>
          </div>

          <div className="interactive-card rounded-[1.75rem] border border-white/10 bg-[radial-gradient(circle_at_top_right,_rgba(214,165,93,0.15),_transparent_32%),rgba(255,255,255,0.04)] p-5 sm:p-6 lg:col-span-3 xl:p-7">
            ...
            <p className="mt-5 text-sm leading-6 text-mist">{dictionary.location.hoursNote}</p>
          </div>
        </div>
      </div>
    </section>
