      ...
      <div className="mt-6 grid gap-4 sm:mt-12 sm:gap-5 md:grid-cols-2 xl:grid-cols-12 xl:auto-rows-[minmax(17rem,_1fr)]">
        {items.map((item, index) => (
          <FeaturedMenuCard
            ...
            index={index}
            locale={locale}
            mobileFeaturedCount={mobileFeaturedCount}
          />
        ))}
      </div>
...
interface FeaturedMenuCardProps {
  ...
  item: MenuItem;
  locale: Locale;
  mobileFeaturedCount: number;
}

function FeaturedMenuCard({ index, item, locale, mobileFeaturedCount }: FeaturedMenuCardProps) {
  const [hasImageError, setHasImageError] = useState(false);
  const hasUsableImage = Boolean(item.imageSrc) && !hasImageError;
      ...
      <div className="h-full overflow-hidden rounded-[1.7rem] bg-midnight/90">
        <div className={`relative overflow-hidden border-b border-white/10 bg-obsidian ${getMenuImageAspectClasses(index)}`}>
          {hasUsableImage ? (
            ...
            <MenuImagePlaceholder category={item.category.es} />
          )}
